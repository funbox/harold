...
module.exports = function generateSnapshot(buildTime) {
  ...
  const currentBranchOrCommit = getCurrentBranchOrCommit();

  return {
    project: projectPackageJson.name,
    ...currentBranchOrCommit ? { gitRef: currentBranchOrCommit } : {},
...
function getCurrentBranchOrCommit() {
  try {
